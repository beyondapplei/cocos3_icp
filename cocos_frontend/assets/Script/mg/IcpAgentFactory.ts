...
function getHttpAgentCtor(): any {
	const HttpAgent = (globalThis as any)?.DfinityAgent?.HttpAgent;
	if (!HttpAgent) throw new Error('DfinityAgent.HttpAgent missing. Ensure lib3/icp-sdk-agent.js is loaded');
	return HttpAgent;
}
...
function createFetchCompat(
	...
	return (async (input: any, init?: any) => {
		// In some transpile/runtime environments, binary bodies may get coerced into
		// Array<number> or other ArrayBufferViews; normalize to Uint8Array to preserve bytes.
		const normalizeBody = (b: any): any => {
			...
			if (typeof ArrayBuffer !== 'undefined' && typeof ArrayBuffer.isView === 'function' && ArrayBuffer.isView(b)) {
				...
				// covers TypedArray
				// @ts-ignore
				if (b.buffer) return new Uint8Array(b.buffer, b.byteOffset || 0, b.byteLength || 0);
			}
			if (Array.isArray(b)) {
				// Best-effort: assume byte values.
		...
		// If the SDK calls fetch with a Request object, do NOT try to feed that Request into
		// RequestInit (it can silently drop/mangle body). Extract a proper init instead.
		...
		// Common case: fetch(urlString, init)
...
export async function createIcpAgent(params: CreateIcpAgentParams): Promise<any> {
	...
	if ((globalThis as any).__icpIdentityDebug) {
		try {
			...
			const pubKeyHex = pubKey ? Array.from(new Uint8Array(pubKey)).map(b => b.toString(16).padStart(2, '0')).join('') : 'N/A';
